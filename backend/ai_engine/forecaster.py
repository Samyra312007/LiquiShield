...
def generate_prophet_forecast(
    ...
    Falls back to linear decay if Prophet is unavailable.
    """
...
def _fallback_forecast(
    ...
    for i in range(forecast_days):
        ...
        mult = 1.0 - (drain_factor * (i / max(forecast_days - 1, 1)))
        current_val = max(0, last_val * mult)
        predicted_dates.append(d.strftime("%Y-%m-%d"))
        forecasted_cash.append(round(current_val, 2))
